...
# Arguments: none
function current_prompt() { echo "$PS1" ; }
...
function reset_prompt() {
    if [ -z "$PROMPT_MANIPULATION_OLD" ] && [ -z "$prompt_manipulation_old" ];
    then
        echo "[ERROR] There was no old prompt to reset to!"
    fi
    if [ -n "$prompt_manipulation_old" ]; then
